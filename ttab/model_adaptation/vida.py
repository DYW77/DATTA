...
class ViDA(BaseAdaptation):
    ...
    def _initialize_trainable_parameters(self):
        ...
        ), "ViDA needs some adaptable model parameters."
    ...
    def initialize(self, seed: int):
        ...
        self._optimizer = self._initialize_optimizer([{"params": model_param, "lr": self._meta_conf.lr},
                                  {"params": vida_param, "lr": self._meta_conf.ViDALR}])
        # self.vida_optimizer = self._initialize_optimizer(vida_params, vida=True)
        self._base_optimizer = copy.deepcopy(self._optimizer)
        # self.vida_base_optimizer = copy.deepcopy(self.vida_optimizer)
        ...
        # self.model_state_dict.to(self._meta_conf.device)
    ...
    def one_adapt_step(
        ...
        # batch.to(self._meta_conf.device)
        with timer("forward"):
            ...
            if uncertainty>= self._meta_conf.unc_thr:

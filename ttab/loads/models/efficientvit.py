...
class CascadedGroupAttention(torch.nn.Module):
    ...
    #             +
    #             (trainingab[i] if self.training else self.ab[i]).to('cuda:1')
    #         )
    ...
    def forward(self, x):  # x (B,C,H,W)
        ...
        # 确保 attention_biases 和 attention_bias_idxs 在同一设备上
        trainingab = self.attention_biases[:, self.attention_bias_idxs].to(device)
        ...
        for i, qkv in enumerate(self.qkvs):
            ...
            attn = (
                (q.transpose(-2, -1) @ k) * self.scale
                +
                (trainingab[i] if self.training else self.ab[i].to(device))  # 确保 bias 在同一设备上
            )
